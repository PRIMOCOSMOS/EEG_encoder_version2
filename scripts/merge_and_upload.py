...
def main():
    ...
    p.add_argument("--pattern", default="part_*",
    ...
    p.add_argument("--state-file", default="",
                   help="Where to persist resumable state (default: <scratch>/_oss_merge_state.json)")
    p.add_argument("--revision", default="master")
    p.add_argument("--token", default="", help="ModelScope token (overrides env)")
    p.add_argument("--max-part-retries", type=int, default=5,
                   help="Retry budget per OSS upload_part call (default 5)")

    # --- legacy / fallback path ---
    p.add_argument("--use-lfs", action="store_true",
    ...
    p.add_argument("--chunk-size-mb", type=int, default=16, help="(LFS mode) read chunk for SHA-256")
    p.add_argument("--buffer-size-mb", type=int, default=16, help="(LFS mode) upload buffer")
    p.add_argument("--skip-verify", action="store_true", help="(LFS mode) skip re-list verification")

    # --- cleanup helper ---
    p.add_argument("--abort-in-progress", action="store_true",
    ...
    if not token:
        print("[ERROR] No ModelScope token in --token or MODELSCOPE_API_TOKEN env. Cannot upload.",
              file=sys.stderr)
        sys.exit(2)
    ...
    if args.abort_in_progress:
        ...
        n = abort_in_progress_upload(
            dataset_id=args.dataset, path_in_repo=args.path_in_repo,
    ...
    print(f"  state_file       = {args.state_file or (Path(args.scratch_dir) / '_oss_merge_state.json')}")

...
训练模式（--training-mode）：
  cls_only           只训练分类，强度头冻结（默认）
  joint              全程联合训练
  pretrain_then_joint 预训练 + 联合训练
...
from src.trainer import TrainConfig, run_training_per_subject, VALID_TRAINING_MODES
...
def parse_args() -> TrainConfig:
    ...
    p.add_argument("--subjects",   type=str, default="",
                   help="Comma-separated subject IDs (default: all .npz)")
    ...
    p.add_argument("--freeze-intensity-head", action="store_true",
                   help="（已废弃，请使用 --training-mode cls_only）")

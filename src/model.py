...
模型选择（通过 --model-type 参数）：
  eegnet    → EEGNetDualHead （主力模型，轻量）
  conformer → EEGConformerDualHead（≈0.75M 参数，保留备用）
...
# =============================================================================
# EEGConformer (Song et al., 2022 — adapted for SEED-VII dual-head, 保留备用)
# =============================================================================

class ConvBlock(nn.Module):
    """Temporal + Spatial conv block used before Transformer encoder."""
    def __init__(self, in_ch: int, time_kernel: int = 20, time_padding: int = 10,
                 pool_kernel: int = 60, pool_stride: int = 12, dropout: float = 0.5):
        super().__init__()
...
class EEGConformerDualHead(nn.Module):
    ...
    def __init__(self, Chans: int = 62, Samples: int = 800, embed_dim: int = 40,
                 time_kernel: int = 20, time_padding: int = 10,
                 pool_kernel: int = 60, pool_stride: int = 12,
                 transformer_layers: int = 6, transformer_heads: int = 10,
                 ffn_dim: int = 160, head_hidden: int = 256,
                 dropout: float = 0.5, nb_classes: int = 7, int_hidden: int = 64):
        super().__init__()
        self.conv_block = ConvBlock(in_ch=1, time_kernel=time_kernel, time_padding=time_padding,
        ...
        self.fc = nn.Sequential(
            nn.LayerNorm(embed_dim), nn.Linear(embed_dim, head_hidden),
    ...
    def _extract_features(self, x: torch.Tensor) -> torch.Tensor:
        x = self.conv_block(x)                     # (B, 1, 62, T')
        x = x.permute(0, 3, 2, 1).squeeze(-1)      # (B, T', 62)
...
def build_model(model_type: str, cfg: dict) -> nn.Module:
    ...
    if model_type == "eegnet":
        return EEGNetDualHead(
            Chans=cfg["n_channels"], Samples=cfg["n_timepoints"],
            F1=cfg["F1"], D=cfg["D"], F2=cfg["F2"],
            kernLength=cfg["kernLength"], dropout=cfg["dropout"],
            nb_classes=cfg["n_classes"],
            int_hidden=cfg.get("intensity_head_hidden", 64))
    elif model_type == "conformer":
        return EEGConformerDualHead(
            Chans=cfg["n_channels"], Samples=cfg["n_timepoints"],
            embed_dim=cfg["embed_dim"], time_kernel=cfg["time_kernel"],
            time_padding=cfg["time_padding"], pool_kernel=cfg["pool_kernel"],
            pool_stride=cfg["pool_stride"],
            transformer_layers=cfg["transformer_layers"],
            transformer_heads=cfg["transformer_heads"],
            ffn_dim=cfg["ffn_dim"], head_hidden=cfg["head_hidden"],
            dropout=cfg["dropout"], nb_classes=cfg["n_classes"],
            int_hidden=cfg.get("intensity_head_hidden", 64))
    else:

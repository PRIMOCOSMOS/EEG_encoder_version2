...
训练模式（training_mode）：
  "cls_only"          : 全程只训练分类头，强度头参数冻结，损失只有 L_cls
  "joint"             : 全程联合训练（L_cls + L_reg），无预训练阶段
  "pretrain_then_joint": 前 pretrain_epochs 只开 L_cls，之后联合训练（原始行为）

两种训练入口：
  run_training(cfg)              全被试混合，一个模型
  run_training_per_subject(cfg)  每个被试独立训练，N 个模型
"""
from __future__ import annotations

import gc, json, logging, math, os, random, time, copy
...
from .dataset import (
    EEGMmapDataset, MmapXStore,
    filter_by_subjects, scan_npz_metadata, split_trials_from_meta,
)
...
# 合法训练模式
# --------------------------------------------------------------------------
VALID_TRAINING_MODES = ("cls_only", "joint", "pretrain_then_joint")
...
class TrainConfig:
    ...
    # ★ 训练模式（替代旧的 freeze_intensity_head 布尔开关）
    #   "cls_only"          : 强度头冻结，全程只训练分类，损失只有 L_cls
    #   "joint"             : 全程联合训练（L_cls + L_reg）
    #   "pretrain_then_joint": 前 pretrain_epochs 只开 L_cls，之后联合（原始行为）
    training_mode: str         = str(TRAIN_DEFAULTS.get("training_mode", "cls_only"))
...
def make_balanced_loader(ds, y_all, train_idx, bs, nw, pin=False):
...
# --------------------------------------------------------------------------
# 模型初始化：根据 training_mode 决定冻结策略和 optimizer 参数组
# --------------------------------------------------------------------------

def _build_model_and_optimizer(cfg: TrainConfig, device: torch.device):
    """构建模型并根据 training_mode 设置冻结和 optimizer。

    Returns:
        model, optimizer, n_params(int), n_frozen(int)
    """
    arch_cfg = EEGNET_CONFIG if cfg.model_type == "eegnet" else CONFORMER_CONFIG
    model = build_model(cfg.model_type, arch_cfg).to(device)

    if cfg.training_mode == "cls_only":
        # 强度头参数永久冻结，optimizer 只接管分类相关参数
        n_frozen = freeze_intensity_head(model)
        trainable_params = [p for p in model.parameters() if p.requires_grad]
    else:
        # joint / pretrain_then_joint：全部参数可训练
        n_frozen = 0
    ...
    return model, optimizer, n_params, n_frozen


# --------------------------------------------------------------------------
# 损失函数：根据 training_mode 和当前 epoch 决定损失配置
# --------------------------------------------------------------------------

def _build_criterion(cfg: TrainConfig, epoch: int, rse: int) -> WeightedDualLoss:
    """返回当前 epoch 对应的损失函数实例。"""

    if cfg.training_mode == "cls_only":
        # 全程只有分类损失，beta=0 gamma=0
        return WeightedDualLoss(LossConfig(
            alpha=cfg.alpha_cls, beta=0.0, gamma=0.0,
...
def _phase_tag(cfg: TrainConfig, epoch: int) -> str:
    """返回当前 epoch 的阶段标签，用于日志。"""
    if cfg.training_mode == "cls_only":
        return "CLS"
    if cfg.training_mode == "joint":
        return "JOINT"
    return "PRE" if epoch <= cfg.pretrain_epochs else "JOINT"
...
def _run_single_training(
    ...
    # 构建模型 + optimizer（冻结由 training_mode 决定）
    model, optimizer, n_params, n_frozen = _build_model_and_optimizer(cfg, device)
    scaler = torch.amp.GradScaler("cuda", enabled=use_amp)

    logger.info(
        f"{model_label}[MODE] training_mode={cfg.training_mode}  "
        f"params_trainable={n_params:,}  params_frozen={n_frozen:,}"
    )
    ...
    if cfg.balance_train:
        ...
        classes, counts = np.unique(y_all[train_idx], return_counts=True)
        dist_str = ", ".join(f"cls{c}:{cnt}" for c, cnt in zip(classes, counts))
        logger.info(f"{model_label}[BALANCE] {dist_str} → WeightedRandomSampler 均衡各类至等频")
    else:
    ...
    start_epoch = 1
    rse = cfg.pretrain_epochs + 1   # rank_started_at_epoch（仅 pretrain_then_joint 用）
    ...
    for epoch in range(start_epoch, cfg.max_epochs + 1):
        ...
        criterion = _build_criterion(cfg, epoch, rse)
        phase     = _phase_tag(cfg, epoch)
        ...
        logger.info(
            f"{model_label}[E{epoch:03d}|{phase}] lr={lr_now:.2e} "
        ...
        if dh:
            logger.warning(f"{model_label}[TIMEUP] @{epoch}"); break
    ...
    # 测试评估用纯分类损失（不含 reg / rank）
    test_criterion = WeightedDualLoss(LossConfig(
        alpha=1.0, beta=0.0, gamma=0.0,
        label_smoothing=0.0, enable_rank=False,
    ...
    if best_model_path.exists() and test_loader:
        ...
        test_metrics = evaluate(model, test_loader, test_criterion, device, use_amp)
...
def run_training(cfg: TrainConfig) -> Dict:
    ...
    if cfg.training_mode not in VALID_TRAINING_MODES:
        raise ValueError(f"training_mode must be one of {VALID_TRAINING_MODES}, "
                         f"got '{cfg.training_mode}'")

    set_seed(cfg.seed)
    logger = setup_logger(cfg.output_dir)
    logger.info(f"[CONFIG] training_mode={cfg.training_mode}, "
                f"model={cfg.model_type}, balance={cfg.balance_train}")

    active_config = EEGNET_CONFIG if cfg.model_type == "eegnet" else CONFORMER_CONFIG
    if cfg.model_type not in ("eegnet", "conformer"):
    ...
    if bool(train_subj or val_subj or test_subj):
        all_s = sorted(set(str(m.get("subject", "")) for m in meta))
        train_idx = filter_by_subjects(meta, train_subj) if train_subj \
                    else filter_by_subjects(meta, [s for s in all_s
                                                   if s not in set(val_subj)|set(test_subj)])
        if val_subj:
        ...
        if test_subj:
            ...
            n_tst = max(1, int(round(len(pool) * 0.5)))
            test_idx = np.sort(rng.choice(pool, size=min(n_tst, len(pool)), replace=False)) \
                       if len(pool) > 0 else np.array([], dtype=np.int64)
    ...
    if len(val_idx)   == 0: logger.error("Empty val.");   x_store.close(); return {"error":"empty val"}
    if len(train_idx) == 0: logger.error("Empty train."); x_store.close(); return {"error":"empty train"}
...
def run_training_per_subject(cfg: TrainConfig) -> Dict:
    ...
    global_logger.info(f"[CONFIG] training_mode={cfg.training_mode}, "
                       f"model={cfg.model_type}, balance={cfg.balance_train}")
    ...
    global_logger.info(f"[PER-SUBJ] {len(all_npz)} subjects: "
    ...
    for subj_npz in all_npz:
        ...
        if deadline_ts and time.time() >= deadline_ts:
            global_logger.warning(f"[PER-SUBJ] Time limit before subject {subj}.")
            break

        global_logger.info(f"\n{'='*60}\n[PER-SUBJ] === {subj} → {subj_out} ===\n{'='*60}")
        ...
        try:
            ...
            global_logger.error(f"{label}not found: {e}"); continue
        ...
        if len(train_idx) == 0 or len(val_idx) == 0:
            global_logger.warning(f"{label}Skipped: train or val empty."); continue
        ...
        remaining = (deadline_ts - time.time()) / 3600. if deadline_ts else cfg.max_runtime_hours
        subj_cfg  = copy.copy(cfg)
        subj_cfg.output_dir        = subj_out
        subj_cfg.max_runtime_hours = max(0.0, remaining)
        ...
        global_logger.info(
            f"{label}Done. val_acc={summary['best_val_acc']:.4f}, "
            f"test_acc={summary.get('test',{}).get('acc', float('nan')):.4f}, "
            f"test_mae={summary.get('test',{}).get('intensity_mae', float('nan')):.4f}"
        )
        x_store.close(); gc.collect()

    if not all_summaries:
        return {"error": "no subjects trained"}

    test_accs = [v["test"]["acc"] for v in all_summaries.values()
                 if v.get("test") and "acc" in v["test"]]
    test_maes = [v["test"]["intensity_mae"] for v in all_summaries.values()
    ...
    global_logger.info(f"\n{'='*60}\n[PER-SUBJ] ALL DONE — {len(all_summaries)} subjects")
    if test_accs:
        global_logger.info(
    ...
    if test_maes:
        global_logger.info(f"[PER-SUBJ] Test MAE: mean={np.mean(test_maes):.4f} "

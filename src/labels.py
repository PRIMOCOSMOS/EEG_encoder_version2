...
# 7-class label mapping (固定整数索引，便于训练)
...
def trial_id_to_emotion(session_id: int, trial_id_1based: int) -> str:
    ...
    Returns the emotion code (e.g. "H", "S").
    """
...
def build_full_session_label_list(session_id: int) -> List[Tuple[int, str, int]]:
    """Returns list of (trial_id_1based, emotion_code, class_idx) for the session."""
    out: List[Tuple[int, str, int]] = []
    for tid in range(1, TRIALS_PER_SESSION + 1):
        code = trial_id_to_emotion(session_id, tid)
        out.append((tid, code, EMOTION_TO_IDX[code]))
    return out


def build_dataset_label_list() -> Dict[int, List[Tuple[int, str, int]]]:
    """For each of the 4 sessions, return the trial->label list.

    Note: SEED-VII has 20 subjects × 4 sessions × 20 trials = 1600 trials.
    The provided 1-20.mat naming in Design.md (with internal fields '1' .. '80')
    concatenates the 4 sessions' 20 trials into 80 per subject. We provide a helper
    `trial_field_to_session_trial` to map field-name index (1..80) -> (session, trial).
    """
    return {sid: build_full_session_label_list(sid) for sid in (1, 2, 3, 4)}


def trial_field_to_session_trial(field_id_1based: int) -> Tuple[int, int]:
    """Map .mat内字段名 1..80 -> (session 1-4, trial 1-20)."""
